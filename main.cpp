...
SPIFBlockDevice bd(SD_MOSI, SD_MISO, SD_CLK, SD_CS);
 ...
 *  OBD Init
 */
uint16_t allowedPIDs[12] = { 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF,
...
const parseFunc parseFuncs[] = { NULL,
				 parseMIL,
				 NULL,
				 NULL,
				 parsePercentage,
...
int main()
{
	...
	sendData(telitData);
	telitData =
		"AT#HTTPCFG=0,\"https://fleethive-ddb78.firebaseio.com/.json\",80,0,\"\",\"\",0,120,1";
	sendData(telitData);
	telitData = "";
...
void saveTimestamp()
{
	...
	if (package) {
		...
		gps.crack_datetime(&year, &month, &day, &hour, &minutes,
				   &second, &hundredths);

		char timestamp[25];
		sprintf(timestamp, "%02d-%02d-%02dT%02d:%02d:%02d-03:00", year,
...
float distanceBetweenCoordinates(float latitude1, float longitude1,
...
{
	int16_t iPID = parsePID(
		PID); // Use the PID to know what function to use for parsing
	char *response;
	obdSerial.printf("%s\n", PID.c_str());
...
char *getResponse(void)
{
	...
	while (inChar != '\r') {
		if (obdSerial.readable() >
		    0) { // There is something to be read on the Serial port
			inChar = obdSerial.getc();
			if ((inChar == 0x3E) || (inChar == '\0') ||
			    (inChar == '\r')) {
				response[index] = '\0';
				index = 0;
			} else {
